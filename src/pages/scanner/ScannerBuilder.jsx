...
import { Card, Form, Button, Badge, Stack, Modal } from "react-bootstrap";
import {
  operatorMap,
  timeframeMap,
  OPERATORS,
...
import EditableMultiSelect, {
  EditableNumber,
  EditableSelect,
} from "../../components/indicator/EditTableLabel";
import AlertModal from "../../components/scanner/ScannerModals";
import IndicatorRuleModals from "../../components/scanner/IndicatorRuleModals";
import IndicatorBuilderListing from "../../components/scanner/IndicatorBuilderListing";
    ...
    const parseSide = (rawStr) => {
      ...
      if (mathMatch) {
        ...
        if (match2) {
          ...
          if (
            typeof MA_INDICATORS !== "undefined" &&
            MA_INDICATORS.includes(obj2)
          ) {
            params.source = "close";
          }
        }
  ...
  const actionButtons = [
    {
      ...
      hover: {
        ...
        boxShadow: "0 8px 22px rgba(147,51,234,0.45)",
        color: "#fff",
      },
      iconHover: "scale(1.2) rotate(-10deg)",
    },
    {
      label: "Save Scan",
      icon: <HiOutlineSave style={{ transition: "transform 0.22s ease" }} />,
    ...
    },
    {
      ...
      iconHover: "rotate(-180deg) scale(1.1)",
    },
    {
      label: "Create Alert",
      icon: <GoAlertFill style={{ transition: "transform 0.3s ease" }} />,
      ...
      iconHover: "scale(1.2) rotate(-8deg)",
    },
  ];
        ...
        <Card className="border-0 shadow-none mx-4 my-1">
          <Card.Body className="d-flex flex-column gap-3">
            ...
            <Stack direction="horizontal" gap={3} className="pt-3 flex-wrap">
              {actionButtons.map(
                ({ label, icon, title, onClick, style, hover, iconHover }) => (
                  <Button
                    key={label}
                    title={title}
                    onClick={onClick}
                    className="d-flex align-items-center gap-2 px-4 py-2 fw-semibold border-1"
