  ...
  return (
    <div style={{ padding: "4px 0" }}>
      {rows.map((row) => (
        ...
        >
          {/* CHECKBOX + LABEL */}
          <Col className="ps-3 pe-1 py-2" style={{ minWidth: 0 }}>
            <Form.Check
              type="checkbox"
                ...
                background:
                  row.type !== "fill"
                    ? (selectedStyle?.[row.key]?.color ??
                      row.color ??
                      "#2962ff")
                    : (selectedStyle?.[row.key]?.topFillColor1 ??
                      selectedStyle?.[row.key]?.bottomFillColor1 ??
                      row.topFillColor1 ??
                      row.bottomFillColor1),
          ...
          {/* INPUT COLUMN — fixed width, placeholder if no value */}
          <Col
            xs="auto"
